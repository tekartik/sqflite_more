  ...
  if (context != null) {
    ...
    group('raw', () {
      ...
      test("Options", () async {
        ...
        String path = await context.initDeleteDb("raw_query_format.db");
      ...
      */
      test("Transaction", () async {
        String path = await context.initDeleteDb("simple_transaction.db");
        Database db = await factory.openDatabase(path);
        try {
          await db
      ...
      test("Transaction recursive", () async {
        String path = await context.initDeleteDb("transaction_recursive.db");
        Database db = await factory.openDatabase(path);
      ...
      test("Transaction open twice", () async {
        //Sqflite.devSetDebugModeOn(true);
        String path = await context.initDeleteDb("transaction_open_twice.db");
        Database db = await factory.openDatabase(path);
      ...
      test("Debug mode (log)", () async {
        ...
        String path = await context.initDeleteDb("debug_mode.db");
        Database db = await factory.openDatabase(path);
      ...
      test("Demo", () async {
        ...
        String path = await context.initDeleteDb("simple_demo.db");
      ...
      test('BatchQuery', () async {
        // await Sqflite.devSetDebugModeOn();
        String path = await context.initDeleteDb("batch.db");
        Database db = await factory.openDatabase(path);
      ...
      test('Batch', () async {
        ...
        batch = db.batch();
        batch.rawInsert(
            "INSERT INTO Test (name) VALUES (?)", <dynamic>["item2"]);
        ...
        batch = db.batch();
        batch.rawDelete(
            "DELETE FROM Test WHERE name = ?", <dynamic>["new_item"]);
        batch.delete("Test",
      ...
      test('Batch in transaction', () async {
        // await Sqflite.devSetDebugModeOn();
        String path = await context.initDeleteDb("batch_in_transaction.db");
        Database db = await factory.openDatabase(path);
      ...
      test("Open twice", () async {
        // Sqflite.devSetDebugModeOn(true);
        String path = await context.initDeleteDb("open_twice.db");
        Database db = await factory.openDatabase(path);
        await db
            .execute("CREATE TABLE Test (id INTEGER PRIMARY KEY, name TEXT)");
        Database db2 = await factory.openDatabase(path,
      ...
      test("text primary key", () async {
        // Sqflite.devSetDebugModeOn(true);
        String path = await context.initDeleteDb("text_primary_key.db");
        Database db = await factory.openDatabase(path);
      ...
      test("without rowid", () async {
        ...
        Database db;
        try {
          String path = await context.initDeleteDb("without_rowid.db");
      ...
      test('Reference query', () async {
        String path = await context.initDeleteDb("reference_query.db");
        Database db = await factory.openDatabase(path);
        try {
          Batch batch = db.batch();
